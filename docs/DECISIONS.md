...
**Scope / known gap (step 2).** This covers **scoring relaxation only**. For attributes that drive the candidate search keyword (resistance/capacitance/package via `buildCandidateSearchQuery`), near-value/alternate parts are never fetched, so `range` bands and keyword `set` criteria can't surface new parts until **fetch-widening** (parametric/range queries to Digikey + Atlas) lands. Non-keyword attributes (AEC, etc.) work immediately because those parts are already in the pool. **Acceptance does NOT override the qualification-domain SAFETY gate** (cross-domain block); it IS reconciled with `filterAutomotiveAecMismatches` (which keys off the rule result the short-circuit flips). Full status, the resolved/deferred review findings, and step-2 detail live in [docs/acceptance-criteria-followups.md](acceptance-criteria-followups.md).

**Files.** `lib/types.ts`, `lib/services/acceptanceModifier.ts` (new), `lib/services/matchingEngine.ts`, `lib/services/partDataService.ts`, `lib/services/partDataCache.ts`, `app/api/xref/[mpn]/route.ts`, `lib/api.ts`, `hooks/useAppState.ts`, `components/{AttributesPanel,DesktopLayout,AppShell}.tsx`, tests. Branch `feat/source-attribute-tolerance`. 2079 tests pass.
