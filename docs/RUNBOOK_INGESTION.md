...
| 5-star | **Sweep the ⭐ rows first**: filter to `AI: ACCEPT`, tick the starred rows (header checkbox ticks every starred row on screen), click **Batch Accept (N)**. | A star = AI verdict `accept` + **high** confidence + *no caveat anywhere in its explanation* (Decision #267). It marks "nothing here to look at" — the fast lane. You still tick each box, all the usual row warnings still render, and an **Undo** appears in the snackbar for the whole batch. Rough yield: ~40% of a fresh Generate batch comes back `accept`, ~85% of those are starred. |
