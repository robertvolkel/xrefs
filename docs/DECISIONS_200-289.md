# Architectural Decisions — Archive (Decisions 200–289)

> Full text of decisions 200–289. The index lives in [docs/DECISIONS.md](DECISIONS.md).
...
## Decision #283 — Atlas coverage is one per-product definition (Data + Reach), and the numbers stay current (July 28, 2026)

**Symptom.** After a clean "Refresh from accepts" backfill (437,093 scanned / 23,084 changed / 0 errors), the Manufacturers-page headline stayed at 36%. It was *already current* — the cache had recomputed after the backfill finished — but the number is structurally unable to move.

**Root cause.** The Manufacturers route derived coverage from `get_manufacturer_product_stats`'s `param_keys` = `ARRAY_AGG(DISTINCT k)` over a whole `(manufacturer, family)` group, then credited each rule attribute to *every* product in the group if *one* product had it (`covered * count`). An attribute counted as covered for all 20,206 HONGFA relays if a single relay carried it — the metric was saturated, so re-translating products added ~0. Meanwhile the Atlas Coverage page used a *different*, per-product formula (`get_atlas_coverage_aggregates`, `p.parameters ? rule_attr` per row), so the two pages disagreed for the same manufacturer by a measured **+10.4 percentage-point** mean (higher on 259 of 287, lower on 0; ISC 58% vs 31%, YANGJIE 67% vs 40%).

**Fix — one definition, reported as two numbers.**
- **Shared rollup** ([lib/services/atlasCoverageAggregates.ts](../lib/services/atlasCoverageAggregates.ts)): `buildFamilyAttrsPayload` (moved out of the atlas route) + the pure `rollupCoverageByManufacturer(rows)` over `get_atlas_coverage_aggregates`. Both admin routes feed the same function, so per-MFR and headline coverage can't drift; the unit test on the pure function is the anti-drift guard. Verified live: the two RPCs return identical 408-manufacturer key sets and identical total/scorable product counts, so the swap can't drop an MFR and Reach agrees regardless of source.
- **Two headline numbers**, because two different gaps respond to two different kinds of work: **Data coverage** = covered attribute slots / required slots over classifiable products (**33%** = 2,052,794 / 6,288,428 — the number dict accepts + backfill move); **Reach** = classifiable / all products (**69%** = 288,989 / 416,822 — moves only when families/classifiers are added; the other 127,833 products can't be cross-referenced at all). They multiply to the whole-catalog truth (22.6%). Both surfaced on the Manufacturers summary and as Atlas-page KPI tiles with shared, live-number tooltips ([atlasCoverageCopy.ts](../lib/services/atlasCoverageCopy.ts)). The old unweighted-mean-of-per-MFR-% headline (`avgCoveragePct`) is gone. `manufacturers-list` cache bumped v5→v6.
- **`param_keys` retired.** The `param_union` CTE (a `LATERAL jsonb_object_keys` unnest over ~500K key-rows, the sole reason for the RPC's 120s timeout) was removed from `get_manufacturer_product_stats`.

**Fix — the coverage drawer was doubly wrong.** It read `atlas_products` with a plain `.select().eq('manufacturer', name)` — capped at PostgREST's 1000 rows (66 groups exceed it; HONGFA/F1 has 20,206 products), AND its only live caller (the MFR detail page) passed the canonical `nameDisplay` ("ISC 无锡固电") while products store `name_en` ("ISC"), so variant-stored MFRs matched *zero*. New `get_atlas_family_attr_coverage(text[], text) RETURNS jsonb` counts in Postgres (cap-proof); the route resolves variants via `resolveManufacturerAlias().variants` and passes them all.

**Fix — the numbers now stay current.** (a) The backfill completion handler invalidates the **atlas-coverage** cache too, not just the MFR list (it re-translates coverage on both pages; sibling ingest routes already did both). (b) `atlas-ingest.mjs`'s three cache-purge sites deleted the literal `'manufacturers-list'` — a **silent no-op since the key was versioned** — now a version-proof `.like('key','manufacturers-list%')`. (c) All three cache routes' background `computeAndPersist` now populate **L1** (not just L2) through a newer-guarded `setMemCache`, so a recompute can't be shadowed by the 60s hot cache, and a slow stale read can't clobber a fresh recompute; the Manufacturers route gained the 6h SWR backstop the other two already had. (d) The panel's post-backfill refresh **polls until the served `cachedAt` passes the run's finish time** (showing "Updating coverage…"), instead of firing one immediate re-fetch that flashed the pre-backfill number.

**One expected regression:** every coverage % on the Manufacturers page drops (headline 36% → 33% Data + 69% Reach; per-MFR ~−10pt to match the detail page). That's the correction landing — the number stops flattering itself — plus Reach is genuinely new information (that 31% of the catalog is unreachable was invisible before). Admin-reporting only; no matching-engine change.

**Rollout ordering (load-bearing):** the `get_manufacturer_product_stats` trim must NOT be applied to the live DB before the new code deploys — the old route reads `param_keys`, so trimming it first zeroes live coverage. The new `get_atlas_family_attr_coverage` RPC is additive (no live caller until deploy) and safe to apply anytime. Orphaned `manufacturers-list-v2/-v3/-v4` cache rows are deleted as part of the same coordinated apply.

## Decision #284 — Atlas translation backfill re-translates from the DB (`atlas_raw`), not local files, with a non-lossy guard (July 29, 2026)

**Symptom.** "Refresh from accepts" returned **"0 products updated, 1 error"** after ~1.8s, touching nothing. The live status row confirmed it: started 03:32:13, finished 03:32:14, `scanned: 0`, `exitCode: 1`, no progress heartbeat.

**Root cause.** `runBackfillTranslations` re-translated products by re-reading the original vendor source files in `data/atlas/*.json`, starting with `readdirSync('data/atlas/')`. Those files are **gitignored** ("proprietary manufacturer data") so a fresh deploy from GitHub does not carry them, and the deploy that shipped Decision #283 wiped the server's copy. With the folder empty the script exits immediately (0 scanned, exit 1) — the exact failure observed. The workflow depended on ephemeral server-side files that no deploy restores.

**Fix — source the raw params from the database.** Every product already stores its own ingested raw model in `atlas_products.atlas_raw` (`rawModel: model` at ingest — the exact object `mapModel()` consumes); verified **416,822 / 416,822** rows have it with the required nested `category.{c1,c2,c3}` shape. The backfill now:
- Gets the manufacturer list from the already-deployed `get_manufacturer_product_stats` RPC (`RETURNS jsonb`, cap-proof; verified **408 MFRs summing to all 416,822 products** — complete, including unclassified families). **No new SQL.**
- Iterates each MFR's rows via `fetchExistingProducts(mfr)` (already returns `atlas_raw` + `parameters` + `category` + `atlas_source_file`), re-runs `mapModel(row.atlas_raw, …)`, and feeds the result through the **unchanged** tag → merge → compare → chunked-upsert path. `mapModel`'s `parameters` output is independent of the manufacturer/source-file args (they only set the discarded `part.manufacturer`), so DB-sourcing is byte-equivalent to file-sourcing for the same raw model.

Deploy-proof, and unchanged everywhere else: `mapModel`/dictionaries/classifier (golden file unchanged, no `atlasMapper.ts` mirror needed), the route (same `--backfill-translations` flag + summary line), the admin panel's per-manufacturer progress heartbeat, and the cache-invalidation block. The now-dead `fetchExistingForBackfill` (file-path-only lean projection) was removed.

**Non-lossy guard (Decision #278 — no value is ever discarded).** The DB holds ONE `atlas_raw` per `(mpn, manufacturer)`, which can re-map to **fewer** values than what's already stored, two ways seen live: (a) **legacy duplicate-MPN** parts — a source file lists the same MPN twice under two category classifications (e.g. a full-spec TVS Diode AND a sparse Rectifier row, see `dedupRichestByMpn`); `parameters` holds the rich mapping a later backfill wrote, but `atlas_raw` is the sparse occurrence, never rewritten; (b) **dictionary drift** — an older mapping stored more/other keys than today's dictionary emits (e.g. a Zener min/max band the current dict folds away). The file-based backfill dodged (a) by re-deriving the richest occurrence from the file; the DB has only the one raw. The guard: **never let a re-translation reduce a product's non-empty parameter count — keep the richer stored row.** Cheap (runs only on candidate changes), counted + logged as `Preserved N`.

**Verification (full-corpus dry-run diff, old file path vs new DB path).** Old: 567 changed / 437,093 scanned / 20,271 missing. New+guard: **704 changed / 416,822 scanned / 0 missing / 27 preserved** (the Scanned/Missing gap is expected — DB rows vs file models). Findings, all confirmed against live data:
- **401 / 408 MFRs come out identical** to the old file path (same per-MFR changed set); the other **7 differ** — all duplicate-MPN / dictionary-drift cases (INPAQ, TAI-TECH, Hanrun, UMW, AK, SUMIDA, SEMBO). (Comparing per-MFR changed *counts* alone shows 5 differing, but SUMIDA and SEMBO match on count while having preserved rows — so 7 is the true count of differing outcomes.)
- The new path applies **158 value-preserving accept-improvements the old path missed** (INPAQ 67, TAI-TECH 82, Hanrun 7, UMW 2) — the old `dedupRichestByMpn` had picked a duplicate whose raw names don't map, leaving junk `ma`/`v`/`vdc` keys, while `atlas_raw` maps cleanly. Every such change is a re-key with values preserved (`ma=400 → rated_current=400`, `vdc=50 → voltage_rated=50`), verified product-by-product.
...
**Two adjacent fixes were attempted and BOTH reverted — this branch is display-only. Do not reintroduce either.**

1. *Typing mid-load strands the panel.* The confirm flow shares one `AbortController` with the chat turn, so a chat message during `loading-attributes` would kill the in-flight `getPartAttributes` and strand the phase. A separate `attrsAbortRef` scope was built, tested and mutation-verified — and introduced its own regression (the `!sourceAttrs` fallback calls `freshAbort()`, cancelling the user's chat turn). **Both were unreachable:** [ChatInterface.tsx:59](../components/ChatInterface.tsx#L59) puts `phase === 'loading-attributes'` in `inputDisabled`, `SearchInput` honours it in the field, the button AND the submit handler, and the only always-enabled input renders solely on the empty landing screen. A user cannot type while a part loads.

2. *Clicking the source MPN in chat blanks the panel.* Real and user-visible — but the fix was applied to the third branch of `handleMpnClick` (`state.sourcePart`), and the user lands on the **first** (`searchResult.matches`), because a part that was searched for is still in the results. Confirmed inert by browser testing.

**Both are filed in BACKLOG with their reachability facts and the wrong fix site named.** The shared lesson, and the reason this decision records it: **proving a code path is broken is not proving anything reaches it.** Reachability — who can call this, is that entry point enabled, and which branch actually fires — is a separate check, and it was skipped twice in one session. Neither defect was caught by unit tests, mutation testing, or a clean cloud review; both were caught in a browser in under ten minutes.

**The same defect, one surface over: the BOM row popup's comparison went blank for ~10s.** Found in browser testing. Clicking a replacement inside the parts-list popup sets `isComparing`, which unmounts the recommendations list immediately — while the comparison block was gated on `comparisonAttrs && `, so it had not mounted yet. Right two-thirds blank until the fetch returned. `ComparisonView` already had the answer (`isLoadingReplacement` → `OverviewSkeleton`/`CommercialSkeleton`, `replacementError` → an error message) and `DesktopLayout` already used it; the modal simply never wired it. Fix: drop `comparisonAttrs` from the render gate and pass both flags. ⚠️ **The error flag is load-bearing, not decoration** — `handleModalSelectRec`'s catch was EMPTY, so a failed fetch left `modalComparisonAttrs` null forever; keying the skeleton on `!attrs` would have produced an indefinite shimmer, the exact failure this decision exists to prevent. It needed a real loading flag cleared on BOTH outcomes plus a separate error flag. Safe by signature, not by inspection: `replacementAttributes` is typed `PartAttributes | null`, `replPart` falls back to `(replacementAttributes ?? recommendation).part`, and `buildComparisonRows` takes null — so the early-mounted panes render the scored comparison from the recommendation while the full attributes load. 7/7 mutations caught, including re-adding the original gate.
...
## Decision #287 — Don't offer a cross-reference we cannot produce; report how much of a match is real evidence (August 3, 2026)

**Reported as** "the Replacements panel suddenly stopped showing up when I click the button in chat." It had not. The panel works — proven the same day on `LM317T` (reached the comparison screen, which is only reachable by clicking a card *inside* the panel) and by the user on `SMMBT3904LT1G`. It fails for one class of part: one with **no usable published specifications**.

**Grounded in production data, not reasoning.** Across **682 saved conversations, 19 Feb – 3 Aug**, 283 reached the replacements screen; exactly **2** had a spec-less source part — both `MNS2N2222AUB`, both that day, both the failure. The gate lines were **byte-identical since 20–23 February 2026**, so nothing recent broke it. Rate of the underlying data gap: **6 of 4,203 cached Digikey lookups (0.1%)** return zero parameters, spread evenly Mar–Aug.

**Why it read as intermittent.** `MNS2N2222AUB` carried zero specs all morning; at 18:27:43 parts.io gap-fill supplied 4 and cached them. The old panel gate opened the moment the parameter count rose above zero — so the same part, same button, behaved differently either side of that instant.

**Two defects, one class of part.**

1. **The app offered an action it could not perform.** `attachPartCapabilities` computed `logic = isFamilySupported(subcategory)` — "is there a rulebook?" but never "is there anything to compare?". With no comparable value every rule returns the silent full `pass` a missing SOURCE value earns ([matchingEngine.ts](../lib/services/matchingEngine.ts) `evaluateIdentity`/`evaluateThreshold`), so the pipeline returned **48 candidates at 94%** for a hi-rel military 2N2222A, topped by `DTC113ZE` (a digital transistor with built-in bias resistors) and `2SC945`.
2. **The panel could not open for those parts.** [AppShell.tsx](../components/AppShell.tsx) computed `hasAttributes = (sourceAttributes?.parameters.length ?? 0) > 0` and [usePanelVisibility.ts](../hooks/usePanelVisibility.ts) gated on it. `hasAttributes` was doing two jobs — "did the fetch resolve?" (its name) and "does this part have parametric data?" (its implementation). The gate wants the first. Chat still announced the 48.

**The fix.**

- **One capability gate**, pure and unit-tested: `canLogicMatch` ([replacementCapability.ts](../lib/services/replacementCapability.ts)) = rulebook exists AND `countComparableSourceRules(attrs, table) > 0`. It feeds the single `attachPartCapabilities` call site, so the *Replacement Options* button, the typed-request decline, and the chat model's own instructions all follow from one change — no new wiring. `countComparableSourceRules` reuses `ruleCanCompare`, already "the ONE definition" of the un-comparable rule set, rather than restating it.
- **The decline says why.** All four capability axes false has two causes; naming the wrong one invents a product limitation the user never retries past (the same failure mode as the SSR registry-key bug in `digikeyMapper`). Supported family ⇒ "no published specifications … nothing to compare means any match score would be meaningless"; unsupported ⇒ the existing wording. One matching sentence added to the `chat()` capability prompt block.
- **The panel gate asks the question it means:** `sourceResolved = !!appState.sourceAttributes`, renamed through the hook so the two questions cannot be conflated again.
- **Report the evidence, everywhere.** `countComparedSpecs` ([types.ts](../lib/types.ts)) counts `matchDetails` where BOTH sides had a value. Shown under every card's percentage and in the deterministic chat summary. Display-only — **no engine change, no cache-version bump**.

⚠️ **A coverage cutoff was measured and REJECTED — do not reintroduce one.** The obvious repair (flag a part whose specs answer too few rules) fails on real data. Measured over all **174 distinct real source parts** in the logs: the **median part answers only ~60%** of its family's scored rule weight, and commodity parts sit exactly where the bad one does — `LL4148` 26%, `MAX232DRG4` 17%, `2SC1815M-GR` 17% versus `MNS2N2222AUB` 13%. Any ratio that catches the bad parts also strips replacements from everyday ones. **Zero is a capability question; anything above zero is a quality question** — so the gate is zero, and thinness is *reported*, not thresholded.

**Blast radius, measured not estimated.** Running `canLogicMatch` over every real source part in the logs: **208 parts · 0 threw · 34 already unsupported (no change) · 172 keep the button · 2 lose the logic axis** — `ES3GB` (only "Moisture Sensitivity Level" + "Recovery Behavior: Consult datasheet") and `MLM205G` (only MSL). Both genuinely uncomparable; their current 94%-style results are equally fabricated.

**The certified carve-out is real.** A blanket "no specs ⇒ no button" would have buried good answers: `NLAS9041DFT2G` has no usable specs, a **supported** family (C5), and **3 active manufacturer-published crosses**. `logic` false ORs with `mfrCertified` true, so it keeps its button — and it is precisely why the panel-gate fix is load-bearing rather than cosmetic.

**Verification.** 14 tests, **5/5 mutations caught** (gate ignoring specs · counting un-comparable rules · one-sided compared-count · each panel-gate line reverted), restore proven green. Fixtures are real parts with real values, because the whole defect is about which real parts fall on which side of the line.
...
**Follow-up — a self-review of this change found two defects the original pass missed, both in code the tests did not reach.** (1) The repair script's `uid` became block-scoped during a late refactor to scan all accounts, and it is read *after* the deletes: `--apply` would have crashed with a `ReferenceError` **after destroying rows**, skipping the post-repair verification entirely — invisible in dry-run, which exits before that block. The client repair itself was unaffected (it ran on the pre-refactor version, and was verified). (2) `useMasterViews` gained an effect cleanup setting `cancelled = true`, but `initializedRef` blocks the effect from re-running — so **StrictMode's dev double-invoke cancelled the only load attempt** (`reactStrictMode` is unset in [next.config.ts](../next.config.ts), and Next's default is on), leaving an empty dropdown and `isLoading` stuck in dev. The cleanup was removed: a setState after unmount is a no-op in React 18+, so it was guarding nothing and breaking something.

Also from that pass: the script deleted duplicates on **name alone**, which contradicts this decision's own reasoning — same-named views with different contents are legal *because* the unique index was rejected. It now deletes only rows whose `columns` are identical to the survivor's and which are unedited, skips and reports the rest, and no longer aborts the whole fleet run over one edited row. Plus: all three reads paginated (the 1000-row cap — a short `parts_lists` read would leave a list past row 1000 pointing at a deleted view, the exact breakage the script exists to prevent), `--user` with no value now errors instead of silently widening to every account, a lost `is_default` is restored (the partial unique index forbids >1 default, never 0), the survivor's `masterViewOverrides` now win over the abandoned copy's, and post-repair verification distinguishes a dangling reference **this run caused** from a pre-existing one — the first version exited 1 on a perfect repair because of an unrelated pre-existing dangling id.

`fetchMasterViews` — *the function that contained the bug* — had no test; `decideSeedAction`'s suite builds `{ok:false}` by hand and would stay green if the error→`ok:false` mapping regressed. Added and mutation-checked. A zero-row seed is now a retryable abort (it was reported as success, leaving an account with no views); a *partial* seed is logged but deliberately NOT auto-completed on later loads, because an account missing a starter view is indistinguishable from one where the user deleted it on purpose. `getCurrentUserId()` is resolved lazily — only when the account looks empty and a write may follow — so the common path keeps its original single round-trip, and the resolved id is threaded into `createMasterViewSupabase` instead of each create re-fetching it. **The `--apply` path is now exercised end-to-end against synthetic duplicates** (redundant pair + a same-named pair with different columns + a list pointing at the doomed view, including a `masterViewOverrides` entry), then cleaned up: it deletes only the redundant copy, re-points all three reference kinds, and exits 0.
