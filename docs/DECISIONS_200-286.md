...
**One adjacent bug, made *more* visible by the shimmer, fixed here.** `handleConfirmPart(state.sourcePart)` passed no preview, so `buildOptimisticFromSummary` rebuilt from the thin `PartSummary` and discarded the datasheet, quotes and compliance **already on screen** whenever the user clicked the source part's own MPN link in chat. Now passes `sourceAttributesRef.current`.

**A second "bug" was fixed and then REVERTED — do not reintroduce it.** The confirm flow shares one `AbortController` with the chat turn, so a chat message sent during `loading-attributes` would kill the in-flight `getPartAttributes` AND strand the phase (it is preserved when `sourceAttributes` is set, and the optimistic preview always sets it). A separate `attrsAbortRef` scope was built, tested and mutation-verified — and it also introduced its own regression (the `!sourceAttrs` fallback calls `freshAbort()`, cancelling the user's chat turn). **The whole thing was inert:** [ChatInterface.tsx:59](../components/ChatInterface.tsx#L59) `inputDisabled` includes `phase === 'loading-attributes'`, `SearchInput` honours it in the field, the button AND the submit handler, and the only always-enabled input renders solely on the empty landing screen (`isIdle && messages.length === 0`). **A user cannot type while a part loads, so neither the bug nor its regression was reachable.** Reverted for being unreachable complexity in the most delicate part of the state machine, on an otherwise display-only branch. The latent defect is filed in BACKLOG, tied to that `inputDisabled` guard. **Lesson: proving a code path is broken is not proving it is reachable** — the reachability check (who can actually call this, and is that entry point enabled?) is a separate step, and it was skipped.
