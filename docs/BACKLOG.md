...
**Context.** Greenfield guided selection is now system-driven ([guidedSelection.ts](../lib/services/guidedSelection.ts) `nextGuidedStep` + `guided_select` tool + `extractAnsweredSpecs` in [llmOrchestrator.ts](../lib/services/llmOrchestrator.ts)). Checkpoint on `feat/guided-selection-system-driven` (commit `eff202f`); **not merged/validated**. The system owns the checklist, next-step, button-vs-typed, and search trigger; the model only phrases one step. Residual items below.

- **(0) Live consistency validation gate (P1, blocks merge).** Run greenfield requests ("I need a voltage regulator", + a capacitor, a MOSFET, a relay) several times each; require the SAME flow shape every run. Use the new `[chat] tool:<name>` logs to confirm `guided_select` fires each turn and `search_parts` fires on the search directive. Merge to `main` only if it holds.
- **(a) Model-relay reliability → fully system-emitted turn (P2, the escalation path).** v1 still has the model in two spots it could fumble: it must (i) call `guided_select` each guided turn, and (ii) phrase only the one directed spec / call `search_parts` when told. If live testing shows it skipping the tool or adding extra asks, escalate: have the orchestrator EMIT the guided turn directly (deterministic message + system `data.choices`, no model phrasing) and fire the search server-side — removing the model from the question path entirely. Design noted in Decision #262.
- **(b) Extraction cost / could be cheaper (P3).** Each guided turn runs a temp-0 `extractAnsweredSpecs` call (+ the search turn also runs `forceExtractSpecs`). When the flow is clean (one system-asked spec → one user answer), the answer could be paired positionally to the spec the system just asked, skipping the LLM extraction. Keep the extraction as the robust fallback for messy turns (multi-spec answers, the 30V detour).
- **(c) Disambiguation still model-driven (P3).** The first family-split question ("LDO vs switching", "NTC vs PTC") is left to the model (it does it well, and it's one question). For full determinism, add a supertype→sub-family map (reuse the shape of Decision #258 `resolveDiscoveryScope`) so `guided_select` emits the disambiguation chips itself when `resolveFamilyFromText` returns null on a known ambiguous supertype.
- **(d) Sanity-catches are model-driven (P3).** The "30V is high for an LDO" interjection is a model judgment (kept deliberately). Could be made deterministic per-attribute range rules (e.g. LDO output_voltage > ~20V → confirm) so the catch is consistent, but that's a curation effort; model-driven is acceptable for v1.
- **(e) Choice-option label quality (P3).** `parseOptions` derives chip labels from the attributeName parenthetical / `upgradeHierarchy`. The tightened parser rejects garbage ("X/Y Class", "N/P"→typed instead), but some surviving labels are terse (B9 channel_type falls back to typed; some families' option phrasing is engineering-y). Curate per-attribute option lists from [docs/min_attr_sets.md](min_attr_sets.md) / the families doc where a label reads poorly.
- **(f) `get_selection_questions` → `guided_select` rename leftover (cleanup).** The old read-only `get_selection_questions` tool was replaced by `guided_select`; confirm no stale references remain in prompt/tests after merge. The pure `getSelectionQuestions(familyId)` helper is retained (used by both the controller and the admin marker).
