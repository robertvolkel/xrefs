...
| Aug 5 2026 | B6 | Parts filed as B6 BJTs carry a `VO(V)` output-voltage column with reference-voltage values (`1.24`/`2.5`/`30` V — 1.24 ≈ bandgap, 2.5 = a common Vref). `output_voltage` is a C1 (LDO) / C6 (Voltage Reference) canonical, **absent from the B6 logic table** — a plain BJT has no output voltage — so these 387 parts (2 MFRs) are likely misclassified voltage references / regulators. Do NOT map `VO(V)`→`output_voltage` under B6. | Misclassified parts (suspected) | 387 (2 MFRs) | **Not yet live-verified.** A full `family_id='B6'` `atlas_raw` scan (Aug 5 2026) for a VO/output-voltage column returned 0 products — these rows are in a not-yet-applied Triage batch, not yet in live `atlas_products`. Structural signal only (`output_voltage` ∉ B6 rules; values are classic Vref); confirm MFRs + reclassify target when the batch applies. |
