...
## A part with NO specifications scores 94% against 48 replacements (P2) — pre-existing, surfaced August 2026

**Context.** Found while explaining the new "No parametric data available for this part." empty state (Decision #286). A small number of parts resolve from Digikey with a datasheet, prices, distributors and compliance codes — but **zero parametric attributes**. Measured example: `MNS2N2222AUB` (Microchip). Digikey carries it (`150-MNS2N2222AUB-ND`) but never characterised it: `Parameters: 0`, `ProductDescription` is literally the MPN, category stops at the generic `Transistors` bucket rather than a specific family, `NormallyStocking: false`, `QuantityAvailable: 0`, `ManufacturerLeadWeeks: 50`. That is the signature of a special-order low-volume / hi-rel item — Digikey lists it as orderable, not as a catalogued product. Contrast `GRM188R71H104KA93D`: 16 parameters, `CAP CER 0.1UF 50V X7R 0603`, category `Capacitors > Ceramic Capacitors`.

**Rate (measured, not estimated).** 6 of 4,192 real cached Digikey lookups carry an empty parameter list — **0.1%**. The six: `mns2n2222aub`, `bpr35r1j`, `10fpz-sm-tf`, `fsn-22a-10`, `nlas9041dft2g`, `vy1221k31y5sq6tvx`. Rare, but each one produces a confidently wrong answer rather than a degraded one.

**The defect.** `getRecommendations('MNS2N2222AUB')` returns **48 replacements, top ones at 94% match with 18 rules evaluated and only 2 flagged for review** — including `2SC945` (plain general-purpose NPN) and `DTC113ZE` (digital transistor with built-in bias resistors), neither a sane substitute for a hi-rel 2N2222A. The arithmetic is not broken; the cause is [matchingEngine.ts:232](../lib/services/matchingEngine.ts#L232): when the **source** lacks a value, the rule result is `'pass'` (`result: !sourceParam ? 'pass' : 'review'`). That is correct in the normal case — a candidate should not be penalised for a gap on our side, and it is the same invariant Decision #243 relies on so a greenfield search only gates on *stated* constraints. But when the source has **no** attributes at all, every rule passes for the same empty reason and the match % becomes a high-confidence number derived from nothing compared. Net: the Specs tab now honestly says "no data" while the Replacements panel beside it says 94% — the two surfaces contradict each other, and the misleading one is the one a buyer would act on.

**Why it is worse than a low score.** A missing *candidate* value scores `review` (50% credit, visible as "verify from datasheet"). A missing *source* value scores a silent full `pass` with no note, so nothing on the card signals that the comparison never happened.

**Same part class, second symptom — the Suggestions panel never opens (observed in the browser, August 2026).** [AppShell.tsx:16](../components/AppShell.tsx#L16) computes `const hasAttributes = (appState.sourceAttributes?.parameters.length ?? 0) > 0;`, and [usePanelVisibility.ts:42](../hooks/usePanelVisibility.ts#L42) gates the right-hand panel on it: `showRightPanel = recsRevealed && hasAttributes`. A zero-parameter part therefore **cannot ever show the Replacements panel** — while `showRecsAndDeferAssessment` still posts the chat summary. Live result on `MNS2N2222AUB`: chat says *"Found 48 replacement candidates … Top match: DTC113ZE — Galaxy, 94% match. All candidates pass primary rules."* and the panel does not appear. The user is promised 48 replacements and shown nothing. Pre-existing on `main` (line is byte-identical there; neither file was touched by Decision #286). **`hasAttributes` is being used as two different things** — "did the fetch resolve?" (its name) and "does this part have parametric data?" (its implementation). The panel-visibility gate wants the first; every zero-spec part fails the second. Note the two symptoms pull in opposite directions: the scoring issue argues for showing FEWER/unscored recommendations, this one argues the panel should render at all — so fix them together, not independently, or the app will promise results in chat and hide them, or show 48 bogus matches in a panel that finally opens.

**Fix — needs a product decision, do NOT quietly pick one.** The engine change is small; the question is what the user should see. Options: (a) **warn** — keep the results but surface a panel-level banner ("we have no published specifications for this part; these matches are not spec-verified") and suppress the match %; (b) **unscored list** — present candidates as same-family suggestions with no percentage at all; (c) **suppress** — offer no cross-references and explain why, pushing the user to supply specs via the existing missing-attributes form. Note the app already has a mechanism that fits: `sourceAttrsReadyRef` gates replacement-finding against a *partial preview*, so a "source is too sparse to score" gate has precedent. Whichever is chosen, the trigger should be measured (`parameters.length === 0`, or possibly "no scored rule had a source value") rather than a guessed threshold, and it must not fire on the Decision #257 optimistic-preview window — that is a *timing* gap, not a data gap, and is now correctly handled by the shimmer. Cheap first step regardless of option: log these six MPN shapes so the rate can be tracked as the corpus grows.
