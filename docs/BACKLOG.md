...
**Deferred Digikey extensions (P2/P3):** the Digikey keyword fan-out still only covers E-series attrs. Two ways to extend it (Atlas already covers ALL numeric attrs):
- **(a) Value-grid widening for output_voltage + frequency attrs (P2).** These ARE keyword-driving in `buildCandidateSearchQuery` but non-E-series. Needs per-attribute standard-value lists (e.g. common LDO output voltages 1.0/1.2/1.8/2.5/3.3/5.0V; standard crystal frequencies). Add to `ESERIES_ENUMERABLE_ATTRS`-style handling with a custom enumerator per unit class.
- **(b) Full parametric ValueId filtering (P3).** Digikey v4 `FilterOptionsRequest.ParameterFilterRequest` (discrete ValueId membership). Covers EVERY numeric attr precisely + paginates to thousands, but heavy: per-category ParameterId map + a FilterOptions enumeration call to map in-band values → ValueIds + two-call flow. This is the originally-deferred escalation; build only if E-series fan-out + Atlas prove insufficient.
