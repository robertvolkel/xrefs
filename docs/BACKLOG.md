...
- **(c) Disambiguation — mostly deterministic; "diode" is the remaining gap (P2).** `AMBIGUOUS_HEADS` covers regulator→C1/C2, transistor→B5/B6/B9, and **capacitor→12/58/59/64/60** (added after a live test showed the capacitor flow falling to the LLM, which reintroduced numbered questions + ungrounded MFR/ESR prose). Also fixed the same session: the MPN gate (`looksLikeMpn`) false-positives on type words ("Tantalum", "MLCC", "Film") and was bailing fresh type-naming turns to the LLM — now skipped when the message names a part type. **Still open: "diode."** Bare "diode" → null and isn't in the table; the natural sub-types are B1 Rectifier / B2 Schottky / B3 Zener / B4 TVS, but **B2/B4 are variant families NOT reachable via `resolveFamilyFromText`** (B2 "Schottky" → null), so a clicked "Schottky" chip can't re-pin through the current `pinFamily`. Fix needs `pinFamily` to recognize variant-family description keywords (reuse `familyClassifier`) OR resolveFamilyFromText entries for B2/B4. Until then, "I need a diode" → Schottky stays LLM-driven (the capacitor bug, for diodes). Also: bare "thermistor" resolves straight to NTC (67) rather than asking NTC-vs-PTC — add to the ambiguity set if the silent default proves wrong.
