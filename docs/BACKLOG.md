...
## Atlas duplicate-MPN dual-category collisions — deterministic resolution (Decision #233, June 11 2026)

**Problem.** ~200+ Atlas products across ≥13 MFRs (TAI-TECH, INPAQ, YJYCOIN, SEMBO, LGE, Hanrun, Aosong, SUMIDA, …) appear **twice in one source file under the same `componentName` but two different category classifications** — most commonly **TVS Diodes (B4)** and **Rectifiers (B1)** for the same part (e.g. SEMBO `SA5.0A`). `atlas_products` keys on `(mpn, manufacturer)`, so ingest/backfill map both occurrences and write both against the same row — **last-write-wins**, in source order. Net effects: (a) the row stores whichever occurrence is processed last (often the *sparser* one, stripping good params); (b) backfill/triage can **never converge** these rows (`--backfill-translations --dry-run` permanently reports them as "would change" — the non-winning occurrence is always a pending diff); (c) family-volume stats double-count. This is the residual ~244-row non-convergence found while completing the Decision #231 full-fleet backfill.

**Why deferred.** Picking a winner is a real semantic call, not a mechanical dedup. Likely fix: at map time, when two occurrences share `componentName`, **collapse to one** preferring the **richer / more-specific classification** (more mapped keys, or a deterministic c3 priority e.g. TVS > Rectifier for protection parts) — or merge non-conflicting params across both. Must be **deterministic** (no last-wins) and **mirrored** into `scripts/atlas-ingest.mjs` per the #174 mirror convention. Ties to Decision #175 (B1↔B4 `reclassifyByParameterSignals`), #191 (MPN-quality validator — this is arguably a new validator kind: `duplicate_mpn_multi_category`), #225 (name-string identity / atlas_id spine). A read-only survey script (count dual-category dups per MFR) is a cheap first step.

**Interim state.** The Decision #233 ORDER-BY loader fix (the *separate* non-determinism bug) shipped; these collision rows currently hold whatever their last backfill write left (SEMBO `SA5.0A` is in its good 7-key TVS state). Do **not** re-run backfill on these 13 MFRs to chase convergence — it can't converge and risks stripping the rich version. Smaller residual (~43 rows: MXChip/UMW/Geehy/COSINE/ETA, no dups) is an un-investigated value-level diff — check before assuming it's the same cause.
