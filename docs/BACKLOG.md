...
- **(c) Disambiguation — partly deterministic now (P3).** Known-ambiguous heads (regulator→C1/C2, transistor→B5/B6/B9) are handled deterministically in `decideGuidedTurn` (`AMBIGUOUS_HEADS`). Other unrecognized/ambiguous types ("capacitor") still fall to the LLM to clarify. Extend `AMBIGUOUS_HEADS` (or reuse Decision #258 `resolveDiscoveryScope`) to cover capacitor/diode/inductor families for full determinism. Also: bare "thermistor" currently resolves straight to NTC (67) rather than asking NTC-vs-PTC — add it to the ambiguity set if the silent default proves wrong.
