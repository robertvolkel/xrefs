...
## Parametric-widening convergence — two divergent copies + a latent GHz/THz/negative-band bug (P2) — deferred follow-up to the connector abstraction

**Context.** "Parametric widening" is the Decision #238 acceptance-criteria feature: when a user loosens a spec's acceptance range (e.g. "any voltage ≥ what I need", "resistance within an E-series step"), the candidate FETCH widens to pull in more in-band parts from Digikey's parametric filter, not just exact-value neighbors. That widening logic exists today as **two independent copies** that were written separately and have drifted:
- **greenfield / descriptive-search path** — `fetchGreenfieldParametricProducts` (`lib/services/greenfieldParametricFetch.ts`) uses `parametricFilterSearchMulti` (multi-parameter AND) and the value parser `leadingMagnitudeToBaseSI` (from `searchConstraints.ts`, aliased locally as `facetValueToBaseSI`).
- **recs cross-ref path** — `fetchDigikeyCandidates` (`lib/services/partDataService.ts` ~:2236) uses `parametricFilterSearch` (single-parameter) and the value parser `extractNumericValue`.

The two parsers select **different facet values for GHz/THz and negative-value bands** — a genuine latent correctness bug: the same loosened spec on a high-frequency part (crystals, oscillators, RF) or a negative-value band can pull a different candidate set depending on which path ran. It is **latent** (not reported, not breaking common cases — only edge specs, and only when a spec is loosened) but real. There is already an in-code marker of the intended fix: `// TODO(post-validate): converge this inline block with applyParametricFilter() … Kept inline here to leave the shipped #238 path untouched.` (`partDataService.ts:2342`) — note `applyParametricFilter()` **does not exist yet**.

**Why it was deferred (not done during the connector abstraction).** Unlike every insulation phase (each a *byte-identical* move, provable via the characterization harness), this is a **behavior change** — the highest-risk step — so it cannot ride the "prove it's identical" method and needs its own differential A/B measurement against the shipped Decision #238 behavior, plus its own review. Two nuances to preserve: (a) the recs widening is deliberately **single-parameter** — symmetry with Atlas's single-attribute numeric-band RPC (`fetch_atlas_candidates_widened`), so the `conjunctive` arity (`false` for recs) must be kept; forcing multi-param is a *separate* decision, not part of the merge; (b) the shipped #238 path must not regress.

**Fix.** Route both copies through ONE shared implementation on the `ParametricCapability` seam (`lib/services/providers/types.ts`) — the connector work landed the primitives (`resolveCategoryRefs` / `discoverFacets` / `filterByValues`) and named the higher-level method to add: `applyParametricWidening` (discover → select in-band values via the SINGLE shared parser → apply → re-verify). Standardizing on one parser is what fixes the GHz/THz/negative-band divergence. Suggested plan of record: extract `applyParametricFilter()` / `applyParametricWidening`, port both call sites, keep `conjunctive` per-path, then A/B a corpus of high-frequency + negative-band + normal loosened-spec searches (old vs new) to confirm the ONLY differences are the intended parser fix. The connector abstraction deliberately built this clean home; this item is the work that slots into it.
...
## System-driven guided selection — follow-ups (Decision #262) (P2/P3)

**Context.** Greenfield guided selection is now **fully system-driven (Phase 3)** — the model is BYPASSED on guided turns by `decideGuidedTurn` ([guidedSelectionController.ts](../lib/services/guidedSelectionController.ts)) at the top of `chat()`; it emits the fixed-wording question + system buttons or runs `searchParts` itself. `guided_select` tool REMOVED. Checkpoint on `feat/guided-selection-system-driven`; **not merged/validated**. Residual items below.

- **(0) Live consistency validation gate (P1, blocks merge).** Run greenfield requests ("I need a voltage regulator", + a capacitor, a MOSFET, a relay, an NTC thermistor) several times each; require the SAME flow shape every run AND that the result cards carry "Fits your specs / Below spec" chips. Confirm the model is bypassed on guided turns (no LLM phrasing). Merge to `main` only if it holds.
- **(a) Fully system-emitted turn — DONE (Phase 3).** The escalation from the prior checkpoint shipped: the model no longer phrases guided questions or fires the guided search. Kept here only as the historical record; nothing left to do.
- **(b) Extraction cost / could be cheaper (P3).** Each guided turn runs a temp-0 `extractAnsweredSpecs` call. When the flow is clean (one system-asked spec → one user answer), the answer could be paired positionally to the spec the system just asked, skipping the LLM extraction. Keep the extraction as the robust fallback for messy turns (multi-spec answers).
- **(c) Recognition — now registry-backed (Phase 4, DONE; the whack-a-mole fix).** The recurring "missing word → LLM freelances" pattern is closed: deterministic recognizer first (`AMBIGUOUS_HEADS` covers regulator/transistor/capacitor/diode with `LABEL_TO_FAMILY` re-pinning incl. variant families B2/B4; MPN gate no longer bails on type words), then `classifyPartTypeFamily` — a forced-tool classifier whose enum is the live 43-family registry, output-bounded to a familyId/'none' (no freelancing). Any phrasing of a known component is owned; only theory/MPN/non-sourcing defer. **Residual (P3):** the classifier can't express "ambiguous," so a novel multi-family supertype NOT in `AMBIGUOUS_HEADS` (e.g. a bare "relay" → F1 vs F2) gets the classifier's single best pick rather than a disambiguation — add relay (F1/F2) and any other genuine multi-family supertypes to `AMBIGUOUS_HEADS` as they surface. Also: bare "thermistor" resolves straight to NTC (67) rather than asking NTC-vs-PTC — add to the ambiguity set if the silent default proves wrong. **Cost (P3):** the classifier is one temp-0 call, fired only on fresh sourcing turns the deterministic layer misses (common types never pay it).
- **(d) Sanity-catches dropped for v1 (P3).** The "30V is high for an LDO" nudge is gone (it was model judgment; removing the model removed it). An off value now shows as "Below spec" instead. Re-add as DETERMINISTIC per-attribute range rules (e.g. LDO output_voltage > ~20V → a fixed "unusually high — search anyway?" confirm) if users miss the up-front nudge.
- **(e) Choice-option label quality (P3).** `parseOptions` derives chip labels from the attributeName parenthetical / `upgradeHierarchy`. The tightened parser rejects garbage ("X/Y Class", "N/P"→typed instead), but some surviving labels are terse. Curate per-attribute option lists from [docs/min_attr_sets.md](min_attr_sets.md) where a label reads poorly.
- **(f) Rename leftover (cleanup).** `guided_select` tool + its handler are removed; the pure `getSelectionQuestions(familyId)` / `nextGuidedStep` helpers are retained (used by the controller + admin marker). Confirm no stale `guided_select` / `get_selection_questions` references remain in prompt/tests after merge.
- **(g) 2nd guided selection in one chat falls to the LLM (P3).** `decideGuidedTurn` only ENTERS a fresh selection from a clean screen (`hasOnScreenContext` false) so post-results refine/filter/compare/pivot stay with the LLM. Cost: starting a brand-new guided selection AFTER results are already showing isn't system-driven (the LLM handles it). To allow re-entry, detect "fresh sourcing intent naming a NEW part type different from what's on screen" and clear the gate.
- **(h) MPN typed mid-questions is re-asked (P3).** During an in-progress selection the MPN gate is skipped (so one-word answers like "Fixed" aren't misread as part numbers). Consequence: if the user pivots mid-flow by typing an actual MPN, the controller re-asks the current spec instead of handing off to a part lookup. Rare; add a mid-flow "looks like a deliberate MPN pivot" escape if it bites.
...
- **(5) Informational "tell me about X" question answered by a bare card dump (P2, observed July 1, 2026).** User typed *"What can you tell me about lm358"* — an open, informational question about a named part — and got the 50-result search list with the flat deterministic bubble *"I found 50 parts matching your criteria — click the one you'd like to use."* No answer, no framing (not even "LM358 is a dual op-amp"). **Why:** the model routes the ask to `search_parts` (correct — "lm358" is a real family with 50 orderable variants), but the deterministic search-presentation substitution (Decision #242) then **replaces the assistant's prose** with `buildSearchSummary`'s line. That suppression is deliberate — it's the anti-fabrication guardrail that stops the model reciting half-remembered specs (the greenfield A3 leak) — but it also strips any *legitimate, grounded* framing, so an informational question reads as a non-answer. **Gate subtlety:** the substitution is scoped by `!looksLikeMpn(query)` (whole-message) only, NOT `!mentionsMpn(query)` — so a sentence that *names* a part ("...about lm358") still gets the greenfield "found N parts" bubble even though the user is asking ABOUT that specific part (contrast Decision #255, which added `mentionsMpn` to the *spec-vetting* gate but not this presentation gate). **Desired:** keep the cards, but lead with a SHORT **grounded** framing line built from data we already hold — the search results' own category/subcategory + variant/manufacturer count (e.g. "LM358 — dual op-amp; 50 variants across manufacturers, click one for full specs"), NOT model-recalled specs. Mirrors the deterministic-bubble pattern (Decision #173/#242): the framing is composed from real fields, so it can't fabricate. Likely a `buildSearchSummary` enhancement that detects an informational/`mentionsMpn` intent and enriches the one-liner from the top results' family + count, rather than a prompt change (keeps the regression checklist green). Care: don't reintroduce spec prose; category + count only. Related: the `looksLikeMpn`-vs-`mentionsMpn` gate inconsistency is worth reconciling here (same root as Decision #255).

- **(6) Multi-part sourcing paste — named part numbers buried in prose + a bundled instruction, reliably returns nothing (P2, observed July 8, 2026, customer report).** A buyer pasted a real AVL-style request: *"all these are released under one part number TOSHIBA - TPH1R306PL ST- STL220N6F7 INFINEON - BSC016N06NS Name of part : TRANS.FET-N E 60V 100A TDSON-8 SMT find chinese sources"* — three equivalent 60V/100A N-ch MOSFETs across three MFRs, a description line, and a follow-up instruction. Result: reliably **no matches** (*"I couldn't find any parts matching your criteria"* on ~4 of 5 runs; one fluke run returned 13 cards — but all **Infineon/Western**, silently dropping the "find chinese sources" intent). **Verified the parts are fine:** searching `BSC016N06NS` alone loads instantly — it's the surrounding noise (*"all these are released…"*, the `TRANS.FET-N E` abbreviation, three MPNs jammed together) that derails the search. **Root cause — the MPN-in-prose search turn is NOT stabilized.** On a greenfield turn (`!looksLikeMpn && !mentionsMpn`) the server forces a temp-0 `extract_part_specs` and builds a canonical query ([llmOrchestrator.ts:1267](../lib/services/llmOrchestrator.ts#L1267)), so results are stable. On a turn that *mentions* an MPN (`mentionsMpn` true → `isGreenfield` false) the else branch drops partType/constraints and passes the model's **raw** `search_parts` query straight through ([llmOrchestrator.ts:1290](../lib/services/llmOrchestrator.ts#L1290)) — so what gets searched is whatever the model improvises that turn (usually the whole blob or the `TRANS.FET-N E …` fragment → zero hits). Same input → different query → different result, mostly empty. Same "leave routing to in-the-moment model judgment" failure as the rest of this category; distinct from Decision #266, which fixed a *transient-AI-blip* amplifier (the session-latch), NOT the input handling — this fails even with the AI healthy. **Desired behavior (decided with user July 8, 2026):** on the `!looksLikeMpn && mentionsMpn` path, (a) reliably extract the clean part number(s) via a narrow `tool_choice`-forced temp-0 step (mirror `forceExtractSpecs` / `classifyPartTypeFamily`) returning the MPN string(s) + any bundled follow-up instruction — chosen over pure-regex extraction because telling a real MPN from a package code (`TDSON-8`), a value (`60V`), or a brand in free text is error-prone (a naive rule grabs `TDSON-8`); (b) look them up — one → MPN lookup, several → present as pick-from cards (the working multi-card view); (c) carry the bundled "find chinese sources" through the existing bundled-intent machinery (Decision #172, `pendingIntent` + `pendingPostRecsFilterRef`) so once a part loads, cross-references run and filter to **Chinese alternative parts** (user chose Chinese *equivalents*, NOT Chinese *distributors of the same part*). Only this in-between path changes; clean single-MPN and pure-descriptive turns stay byte-identical. **v1 scope limits:** several equivalents shown as cards to pick one (merging the three into one unified cross-ref set deferred); "find chinese sources" resolves to equivalents only (the where-to-buy-in-China reading deferred); confirm `detectQueryIntent`/`detectOriginIntent` recognize the "find chinese sources" phrasing (or extend them) so the bundled intent actually fires. **Process:** chat-behavior change → re-run the chat regression checklist ([docs/chat-prompt-regression-checklist.md](chat-prompt-regression-checklist.md)) before merge. **Altitude note:** this adds a SECOND forced-extraction special-case next to the greenfield one — if a third appears, unify into one "understand this request" extraction returning `{partNumbers[], specs[], instruction}` and route off that, rather than a per-shape extractor each time.
...
|---|---|---|---|---|---|
| Aug 6 2026 | L2 "ICs" | FFC flat-flexible cables + cable assemblies (SHOU HAN et al.) mis-filed under the L2 **"ICs"** category. They carry cable/connector columns — `线数` (wire count), `线规` (wire gauge), `线长` (length), `线径` (wire dia), `连接端数` (connection ends), `插入厚度` (insertion thickness), `间距` (pitch), `引脚数`/`针脚数` (pin count). MPNs are canonical FFC part numbers (`FFC-0.5-4P-50mm`, `FFC-0.5-18P-50mm`). The Triage cable-spec rows (`线数`→`number_of_lines`, `线规`→`wire_gauge`, `插入厚度`→`insertion_thickness_mm`, `连接端数`→`connection_ends`) all originate here — accepting them just decorates mis-filed parts; the real fix is reclassification to Connectors. | Misclassified parts | ≥1,383 (23% of a 6,000-product `category='ICs'` sample) | Grounded live Aug 6 2026: scanned `category='ICs'`, nested `atlas_raw.parameters`; 1,383/6,000 carry a cable column; sample MPNs are FFC cables from SHOU HAN. **The `ICs` category is a MIX** — genuine ICs (I2C buffers, motor drivers, converters) share it, so reclassify SELECTIVELY (signal: `FFC-` MPN prefix + cable columns), do NOT blanket-move the category. Structural note: these products store vendor columns under nested `atlas_raw.parameters`, not top-level (why earlier top-level scans returned 0). |
| Aug 4 2026 | B3 | LRC Zener diodes carry a malformed TVS-spec fragment: three columns `VC @IPP (V)`, `IPP tp=8/20µs (A)`, `Power tp=8/20µs (WATT)`. 293/702 echo the column name as the value; the rest are one boilerplate constant each (VC=`46`, IPP=`0.87`, Power=`40`). Vendor category = "Zener Diodes" on all 702; MPNs are canonical Zener series (1N47xx, LMBZ/LMSZ/LM5Z). These are genuine Zeners with a junk column — **not** misclassified TVS (the AI guessed B4; wrong). | Garbage columns | 702 | Grounded against live `atlas_products` Aug 4 2026 (`family_id='B3'`, `atlas_raw` scan). Accepting `vc`/`ipp`/`ppk` would stamp a fake 46V clamp / 0.87A / 40W surge onto 400+ real Zeners. |
| Aug 4 2026 | C2 (+all) | Chinese Yes/No values (`是`/`否`) leak untranslated to users. Surfaced on C2 `同步整流` (Synchronous Rectification) — the column header can be fixed by mapping to `sync_rect`, but the `是`/`否` values stay Chinese because there is **no value-level 是→Yes / 否→No translation anywhere in the mapper** (grep of `lib/` returns nothing). Likely affects every boolean-style Atlas param, not just this one. | Display hygiene | ~371 sampled on C2 alone | Grounded Aug 4 2026 (`family_id='C2'` sample: 270 `是`, 38 `否`, 63 `-`; mapped keys still raw Chinese `同步整流`/`同步整流器`). Fix = a small shared 是/否→Yes/No value map applied at display; low risk, broad benefit. |
...
- **B6 `VO(V)` → NOT a misclassification (RETRACTED Aug 6 2026).** Logged Aug 5 as "likely misclassified voltage references" on the basis of a Triage snapshot showing values `1.24`/`2.5`/`30` V. A correct live scan (`family_id='B6'`, **nested `atlas_raw.parameters`** — the Aug 5 scan looked only at top-level keys and wrongly returned 0, which I mis-read as "pending batch") found the opposite: the 730 parts are **digital transistors** (DTC/DTA pre-biased BJTs — MPNs `DTC143TCA`, `DTA143TCA`, `MMDTC044TE`), correctly filed in B6. `VO` is their **on-state output voltage** `VO(on)@Io/Ii` (live values `300mV@10mA`, `50`, `-`, and a name-echo garbage value). **No `1.24`/`2.5` value exists in the live data at all** — the snapshot values did not reflect reality. Declining `VO(V)`→`output_voltage` is still correct (it's a transistor on-voltage / partly garbage, not a regulator output), but there is **no family error**. Lesson: this family nests its columns; verify at the nested level before asserting.
...
## Bulk-map the un-accepted gaia (datasheet) params — P3, optional (Decision #276)
After Decision #276 made gaia-prefixed params honor dictionary accepts, ~18,700 gaia rows remain in the Triage queue — the **un-accepted** datasheet long tail. These are machine snake_case names never added to the dictionary; de-prefixing auto-maps at most ~5–10% of impact (measured), so genuinely clearing them needs real mapping decisions. Shape of the job (grounded, impact-first, human-reviewed): rank stems by product impact from `get_triage_unmapped_aggregate`, propose canonical attributeIds via the existing `/suggest` scaffold **with a hard reject of any attributeId not in `getSchemaAttributes(family)`** (the current route only soft-flags — the `plastic_material` hallucination is the cautionary tale), human-approve, write into `atlas-gaia-dicts.json`, then `--rescan-unmapped-params`. Top-500 stems ≈ 62% of impact, top-1000 ≈ 73% — stop at diminishing returns; do NOT slog all 12,664 distinct stems.

---

## A thinly-specified part still shows a confident match percentage (P3) — the residue of Decision #287

**Resolved by Decision #287, do not re-open:** the *capability* half. A part with **zero** comparable specs no longer gets a Replacement Options button (or a typed-request run), the decline explains why, the panel gate no longer hides results that exist, and every card + the chat summary now report **how many specs were actually compared** ("94% · 1 of 18 specs compared"). Measured blast radius: of 208 real source parts, 2 lose the logic axis (`ES3GB`, `MLM205G` — both carry only Moisture Sensitivity Level / "Consult datasheet").

**What is still open.** A part that answers *a few* rules still produces a confident number. `MNS2N2222AUB` answers 2 of B6's 15 scored rules and reports **94%**; the honest "1 of 18 specs compared" now sits beside it, but the headline figure is unchanged. Cause is unchanged too — [matchingEngine.ts](../lib/services/matchingEngine.ts) `evaluateIdentity`/`evaluateThreshold`: a rule whose **source** value is missing returns `'pass'`. That is correct in the normal case (don't penalise a candidate for a gap on our side) and is the same invariant Decision #243's greenfield search depends on, so it cannot simply be flipped.

⚠️ **A coverage cutoff is NOT the fix — it was measured and rejected (Decision #287).** Over all 174 distinct real source parts in the production logs, the **median part answers only ~60%** of its family's scored rule weight, and commodities sit exactly where the bad parts do: `LL4148` 26%, `MAX232DRG4` 17%, `2SC1815M-GR` 17% vs `MNS2N2222AUB` 13%. Any ratio that catches the bad ones strips replacements from everyday parts. Do not reintroduce one.

**Shape of a real fix (unscoped).** Score the percentage over rules that were *actually compared*, so a thin comparison yields a low number instead of a high one — i.e. exclude source-missing rules from `totalWeight` in `evaluateCandidate` rather than granting them full credit. That changes the number on every card in the app and both recommendation cache versions (`RECS_CACHE_SCHEMA_VERSION`, `BASE_RECS_SCHEMA_VERSION`), and it would re-order results, so it needs its own before/after over the real corpus. **Note it interacts with sorting** — `sortRecommendationsForDisplay` uses match % as a primary key (Decision #244).

**Adjacent, cheap, still open.** The "fill in missing specs" form only appears when **6 or fewer** rules are missing ([useAppState.ts](../hooks/useAppState.ts)), so it never fires for exactly the parts that need it most — a part missing all 18 silently skips the one mechanism that could rescue it. Lifting the cap for the sparse case is small and independent.

**Underlying data rate (measured).** 6 of 4,203 cached Digikey lookups carry an empty parameter list — **0.1%** — spread evenly Mar–Aug: `mns2n2222aub`, `bpr35r1j`, `10fpz-sm-tf`, `fsn-22a-10`, `nlas9041dft2g`, `vy1221k31y5sq6tvx`. Signature of a special-order low-volume / hi-rel item: Digikey lists it as orderable, not as a catalogued product.

---

## LATENT: a chat turn during `loading-attributes` would strand the source panel (P3) — currently UNREACHABLE, guarded only by a disabled input

**Context.** `handleConfirmWithLLM` and `handleSearchWithLLM` share ONE `AbortController` (`abortRef` / `freshAbort`, [hooks/useAppState.ts](../hooks/useAppState.ts)). If a chat turn started while the source part's attributes were still loading it would (a) abort the in-flight `getPartAttributes`, and (b) leave the phase on `'loading-attributes'` — `handleSearchWithLLM` preserves the previous phase when `sourceAttributes` is set, and the Decision #257 optimistic preview *always* sets it, while the confirm flow bails on `signal.aborted` without resolving the phase. Net effect would be: canonical attributes never arrive, and the panel sits in its loading state (now visibly shimmering, post-Decision #286) until the chat turn finishes and incidentally sets a phase.

**Why it is NOT a bug today.** The chat input is disabled for exactly this window. [ChatInterface.tsx:59](../components/ChatInterface.tsx#L59): `inputDisabled = isSearching || phase === 'loading-attributes' || phase === 'awaiting-attributes' || phase === 'awaiting-context' || phase === 'finding-matches'`. [SearchInput.tsx](../components/SearchInput.tsx) honours it in three places — the text field, the submit button, and the submit handler itself (`if (trimmed && !disabled)`). The only always-enabled input is the landing one, which renders solely under `isLanding = isIdle && messages.length === 0` and therefore cannot coexist with `loading-attributes`. There are exactly two `SearchInput` instances in the app and nothing else calls `onSearch`. **So no user-reachable path triggers this.**

**History — read before "fixing" it.** A separate abort scope (`attrsAbortRef` / `freshAttrsAbort`) was built for this during Decision #286, complete with tests and 6/6 mutation verification. It ALSO introduced a fresh regression: with the post-await guard narrowed to `attrsSignal`, a *failed* (as opposed to aborted) fetch fell through to the `!sourceAttrs` fallback, whose first statement is `freshAbort()` — cancelling the user's in-flight chat request so their message got no reply. Both the original defect and that regression were unreachable for the same reason. The whole change was reverted as unreachable complexity in the most delicate part of the state machine. Do not rebuild it speculatively.

**When this becomes real.** The moment anyone relaxes `inputDisabled` to let users type while a part loads — a plausible product request ("why can't I ask a question while it's thinking?"). **That change must not ship without fixing the state machine first.** Minimum: give the attribute fetch its own abort scope, cancelled only on a new part / reset / conversation switch; land its result silently (no chat message, no auto-fire) when only the chat signal was aborted, guarded on the part still being the one on screen; and **keep honouring the chat signal on the `!sourceAttrs` fallback path**, or the fallback's `freshAbort()` kills the very chat turn the user just started. The reverted implementation is in git history at `65afe2a` / `0f0ed5f` on `fix/source-panel-loading-states` if it is ever wanted as a starting point.


---

## Clicking a part's own MPN in chat blanks the panel it is already showing (P3) — cosmetic, fix site is NOT the obvious one

**Context.** Click the source part's MPN where it appears as a link in chat and the panel discards everything already on screen — datasheet, distributor count, prices, RoHS — then refills over the ~17s reload. Observed in the browser, August 2026. The values were already in hand; nothing needed to be thrown away. Purely cosmetic (the data returns), which is why it is P3.

**Why the obvious fix does not work — read this before attempting it.** [`handleMpnClick`](../hooks/useAppState.ts) resolves the MPN against **three** sources in order: `searchResultRef.current.matches` → `recsRef`/`allRecsRef` → `state.sourcePart`. Only the **third** branch is the "this is the part already on screen" case, and a fix applied there is almost never reached: if the user searched for the part (the normal way it got on screen) it is still in `searchResult.matches`, so the **first** branch wins and calls `handleConfirmPart(fromSearch)` with **no preview** — `buildOptimisticFromSummary` then rebuilds a thin `Part` from the card and the panel blanks. This exact fix was written during Decision #286, shipped, and reverted after browser testing showed no change. Do not repeat it.

**Fix.** Handle it on the branch that actually fires: when the resolved `fromSearch` part IS the current source part (compare mpn **and** manufacturer — same MPN from a different maker is a genuinely different part, cf. the per-MFR card collision trap), pass `sourceAttributesRef.current` as the optimistic preview. The recommendations branch already passes a rich preview via `buildOptimisticFromRec`, so it needs nothing. **Verify in a browser, not by reading the code** — the reverted attempt looked correct in review and in tests, and was inert in practice.
