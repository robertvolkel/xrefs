...
describe('buildRecsSummary', () => {
  ...
  it('counts the full candidate set, not a displayed subset (Decision #227)', () => {
    // Non-Active + high-fail candidates are still counted — the panel shows them all.
    const obsolete = rec('OBS', 'Lelon', 85, [mkDetail('Capacitance', 'pass')]);
    obsolete.part.status = 'Obsolete';
    const highFail = rec('HF', 'CapXon', 40, [
      mkDetail('A', 'fail', '1'),
      mkDetail('B', 'fail', '2'),
      mkDetail('C', 'fail', '3'),
    ]);
    const out = buildRecsSummary([rec('A1', 'CapXon', 90, [mkDetail('Capacitance', 'pass')]), obsolete, highFail], 'X');
    expect(out).toContain('Found **3** replacement candidates for **X**.');
