...
**Current MVP:** Cross-reference recommendation engine covering 35 passive, discrete semiconductor, and IC families, with Digikey as the primary data source.
...
Each component family has a logic table in `lib/logicTables/` defining its matching rules. All 35 families are encoded:
...
Contains 21 `.docx` files — one per base component family — defining the cross-reference logic rules that were encoded into the TypeScript logic tables. These are the authoritative source documents. The `passive_variants_delta.docx` covers the 6 variant families.
