...
- **Recommendations L2 cache** (Decision #128): `getRecommendations()` caches the full `RecommendationResult` in `part_data_cache` with `cache_tier='recommendations'`, 30-day TTL. Cache key: `rec:<RECS_CACHE_SCHEMA_VERSION>:<mpn>:<sha1-16-of-inputs>` — inputs include overrides, context, prefs, currency, options. Global/cross-user (no user_id). Invalidated by `invalidateRecommendationsCache()` on admin writes that affect scoring (xref uploads, rule/context overrides, manufacturer enable/disable). Bump `RECS_CACHE_SCHEMA_VERSION` in `partDataCache.ts` when scoring logic changes. Pricing/stock refreshed on display via `triggerFCEnrichment()` (unaffected by recs cache).
- **Recommendation pipeline performance** (Decision #98, #138): UI passes pre-fetched `sourceAttributes` to `/api/xref` POST to skip duplicate `getAttributes()` call. LLM assessment and FindChips candidate enrichment are deferred — recs display immediately, assessment streams into chat afterward. FindChips pricing/lifecycle merges in via background `triggerFCEnrichment()` which sorts recs by display priority (via shared `sortRecommendationsForDisplay()` helper), fires a smaller 30-MPN priority chunk first + subsequent 50-MPN chunks in parallel via `Promise.all`, and merges each chunk into state as it returns so the top-of-list cards populate within seconds even under the 60/min FindChips rate limit. `isEnrichingFC` flag drives a `Loading pricing…` placeholder on cards during enrichment. QC logging is fire-and-forget. See `triggerFCEnrichment()` and `showRecsAndDeferAssessment()` in `useAppState.ts`.
