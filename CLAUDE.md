...
- **Search cache source provenance** (Decision #126): `SearchResult.sourcesContributed` tracks which data sources returned results for each cached search. On cache hit, `shouldBypassSearchCache()` checks if a previously-missing source is now available — if so, bypasses cache and re-queries all sources. Prevents stale partial results when a source recovers from an outage. Legacy cache entries (no tags) force re-query on first access (self-healing). Atlas excluded from bypass checks (always available). No schema migration — tags stored in `response_data` JSONB.
