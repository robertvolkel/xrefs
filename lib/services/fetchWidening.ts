...
/** Range-criterion attributes whose ±% band actually widens the candidate FETCH.
 *  Intentionally NARROWER than the UI's `RANGE_ELIGIBLE_ATTRIBUTE_IDS` — many
 *  range-eligible attrs (inductance, impedance) are rescore-only (not keyword-driving
 *  and not yet wired to the Atlas RPC), so widening them would not change the pool. */
...
function unitClassFor(attributeId: string): 'resistance' | 'capacitance' | null {
