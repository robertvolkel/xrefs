...
// ─── Reserved scoring attributeIds (GENERATED) ──────────
// This file cannot import the TS logic tables, so the set is generated by
// `npm run atlas:reserved-ids`. A test pins it against the live registry, so a
// new family or rule cannot silently leave a scoring slot unprotected here.
const RESERVED_ATTRIBUTE_IDS = new Set(
  JSON.parse(readFileSync(resolve(process.cwd(), 'lib/services/atlas-reserved-attribute-ids.json'), 'utf-8')).ids,
);
const RAW_KEY_PREFIX = 'raw_';
...
function storeRawValue(parameters, decodedName, rawValue, preferredId) {
  ...
  // unmapped column called "RDS(on)" slugs to exactly `rds_on` (weight 9), and
  // nothing downstream can tell a vetted value from a raw one. Mirror of the
  // RESERVED_ATTRIBUTE_IDS guard in lib/services/atlasMapper.ts.
  ...
  // Normalize to base SI exactly like every winner path. Using the bare
  // extractor here is what let "80mΩ@10V" store 80 instead of 0.08.
  const parsed = extractNumericWithPrefix(String(rawValue));
  const numericValue = applyUnitPrefix(parsed.numericValue, parsed.parsedUnit);
  parameters[key] = {
